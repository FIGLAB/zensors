...
<html lang="en">
  ...
  <body>
      ...
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
               ...
               <div id="form_container">
                ...
                <h1 class="page-header"><a>Sensor Settings</a></h1>
                <form id="form_sensor_settings" class="appnitro"  method="post" action="sensor/settings/<?=$sensor->device_id?>/<?=$sensor->sensor_id?>">
                  <div class="form_description">
                    ...
                    <ul>
                      ...
                      <li id="li_1" >
                        ...
                        <div>
                          <input id="element_1" name="sensor_name" class="element text medium" type="text" maxlength="255" value="<?=$sensor->sensor_name?>"/> 
                      ...
                      <li id="li_2" >
                        ...
                        <div>
                          <input id="element_2" name="sensor_question" class="element text large" type="text" maxlength="255" value="<?=$sensor->sensor_question?>"/> 
                        </div>
                      ...
                      <li id="li_5" >
                          ...
                          <button id="delete_sensor">Delete this Sensor</button>
                          </li>
    ...
    <script>
     var rawDataPoints = <?=$sensor->sensor_subwindowpoints?>;
     ...
     for (var i=0; i<rawDataPoints.length; i++) {
      ...
      $(document).ready(function() {
          ...
          // Form Values
          /////////////////////////
          var freq = '<?=$sensor->sensor_frequency?>';
          var datatype = '<?=$sensor->sensor_datatype?>';
          var obfs = '<?=$sensor->sensor_obfuscation?>';
          var activated = '<?=$sensor->active?>';
          var datatype_values = <?=$sensor->sensor_datatype_values?>;
          ...
          // Bind Dropdown menus to values from the database
          $('[name="sensor_frequency"] option[value="'+freq+'"]').attr("selected", "SELECTED");
          $('[name="sensor_datatype"] option[value="'+datatype+'"]').attr("selected", "SELECTED");
          $('[name="sensor_obfuscation"] option[value="'+obfs+'"]').attr("selected", "SELECTED");
          $('[name="sensor_active"] option[value="'+activated+'"]').attr("selected", "SELECTED");
          ...
          $('#delete_sensor').click(function(event) {
                ...
                if (confirm('Are you sure you want to delete this sensor?')) {
                    window.location.href = 'sensor/delete/<?=$device_id?>/<?=$sensor_id?>';
                }
          ...
          // Draw Lasso Points
          /////////////////////////
          setupCanvas("sensor/viewport/<?=$sensor->device_id?>/<?=$sensor->sensor_id?>", canvasDataPoints);
          strokeData = canvasDataPoints;

          // Fine touch of detail :)
          <? if($form_submitted=="true"): ?>
                <? if ($update_success=="true"): ?>
                    $("#notifications").append('<p><strong><small>Settings saved.</small></strong></p>').fadeOut( 1000*5, "linear");
                <? else: ?>
                    $("#notifications").append('<p><strong><small>Oops, an error occured. Please try again.</small></strong></p>').fadeOut( 1000*5, "linear");
                <? endif; ?>
          <? endif; ?>
